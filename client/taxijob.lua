...
Taxi_Options.Payment = { -- Pays on drop off of NPC
    flatRate = false,
    flatRateAmount = 100,
    DistanceMultiplier = 0.3, -- Only applies if flatRate = false, pays driver based on multiplying distance from pick up to drop off.
}
...
Taxi_Options.Taxi = {
    ...
    MaxWait = 40, -- Maximum time (in seconds) between calls
}
...
if Config.TaxiJobOn then
    ...
    RegisterNetEvent('angelicxs-CivilianJobs:taxiJob:AskForWork', function()
        if FreeWork or PlayerJob == Config.TaxiJobName then
            if not MissionVehicle then
                ...
                -- Aplicar uniforme automaticamente ao começar o trabalho
                if Uniforms and Uniforms.HasUniform('taxi-driver') then
                    TriggerEvent('angelicxs-CivilianJobs:Uniforms:Apply', 'taxi-driver')
                end
        ...
        elseif Config.OXLib then
            ...
            if lib then
                local alert = lib.alertDialog({
                    header = Config.Lang['taxi_menu_header_1'],
                    content = 'Nome: ' .. info.name .. ' | ' .. Config.Lang['taxi_menu_header_2'] .. info.estimatedFare,
                    centered = true,
                    cancel = true,
                    labels = {
                        confirm = Config.Lang['taxi_menu_yes'],
                        cancel = Config.Lang['taxi_menu_no']
                    }
                })
                ...
                -- Fallback para notificação simples se ox_lib não estiver disponível
                TriggerEvent('angelicxs-CivilianJobs:Notify', 'Corrida disponível: ' .. info.name .. ' - R$' .. info.estimatedFare .. '. Pressione Y para aceitar ou N para recusar.', Config.LangType['info'])
                CreateThread(function()
                    while true do
    ...
    AddEventHandler('angelicxs-CivilianJobs:Main:ResetJobs', function()
        ...
        -- Remover uniforme automaticamente ao terminar o trabalho
        if Uniforms then
