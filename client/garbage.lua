...
local Garbage_Options = {
    Boss = {
        ...
        Name = "Carlos",
        tag = "LIXEIRO",
        message = "👋 Olá, me chamo 😃**Carlos Santos**, mas pode me chamar de **Seu Carlos**.  \nVou te ensinar a como trabalhar aqui na 🗑️ empresa de coleta de lixo. É bem simples!  \nTe daremos o caminhão de lixo e você seguirá a rota marcada coletando o lixo. 🚛  \nPegue os sacos de lixo e jogue no caminhão para completar cada parada.  \nAgora larga mão de preguiça e vai trabalhar! 💼👊"
    },
    Sprite = {
        icon = 318,
        colour = 48,
    ...
    Payment = {
        flatRate = false,
        flatRateAmount = 100,
        DistanceMultiplier = 0.7, -- Only applies if flatRate = false, pays driver based on multiplying distance from pick up to drop off.
         items = true,
        itemList = {
...
if Config.GarbageJobOn then
    CreateThread(function()
        ...
        while true do
            ...
            if Dist <= 50 and not PedSpawned then
                TriggerEvent('angelicxs-CivilianJobs:MAIN:SpawnBossNPC', Garbage_Options.Boss.Model, Garbage_Options.Boss.Location, 'angelicxs-CivilianJobs:GarbageJob:AskForWork', 'angelicxs-CivilianJobs:GarbageJob:HowTo', ' GarbageJob.lua', Garbage_Options)
                PedSpawned = true
        ...
        if Config.UseThirdEye then
            ...
            function ThirdEyeGarbageSearch()
                ...
                for i=1, #Garbage_Options.Objects do
                    local garbage = GetClosestObjectOfType(pos, 10.5, Garbage_Options.Objects[i], false, false, false, false, false)
    ...
    function GarbageRouteManager()
                        ...
                        if not garbagebag then 
                            ...
                            if vDist <= 5 then
                                ...
                                if IsControlJustReleased(0,47) then
                                    ...
                                    -- Dar itens a cada coleta de lixo se configurado
                                    if Garbage_Options.Payment.items then
                                        local chance = math.random(1, 3) -- 33% de chance de encontrar item
                                        if chance == 1 then
                                            local item = Randomizer(Garbage_Options.Payment.itemList, 'angelicxs-CivilianJobs:GarbageJob:PutGarbage')
                                            while not item do Wait(10) end
                                            PaymentItem(item, 'Garbage Job Collecting Trash - angelicxs-CivilianJobs:GarbageJob:PutGarbage')
                                        end
                                    end
                                    break
                                end
        ...
        -- Verificar se deve dar itens após completar a rota
        if Garbage_Options.Payment.items then
            for i = 1, routenumber do
                local item = Randomizer(Garbage_Options.Payment.itemList, 'angelicxs-CivilianJobs:GarbageJob:GarbageRouteManager')
                while not item do Wait(10) end
                PaymentItem(item, 'Garbage Job Route Complete - angelicxs-CivilianJobs:GarbageJob:GarbageRouteManager')
                Wait(1100) -- Delay de 1.1 segundo entre cada item para evitar detecção de anti-cheat
            end
        end
        garbageOnJob = false
        totaldist = 0
